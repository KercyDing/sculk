...
- 密钥轮换现在会原子替换旧密钥，首次创建可安全处理并发调用
- Profile 和密钥文件改为完整写入临时文件后再发布，避免留下截断文件
- Host 建连与认证改为有界并发处理，并为认证握手增加超时
- 丢弃 `IrohTunnel` 时会通知后台任务退出并关闭 endpoint
- 永久认证拒绝不再进入无限重连
- 修复 `event_delay` 非零时 RTT 更新绕过节流的问题
- 慢事件消费者不再阻塞隧道网络控制流程
